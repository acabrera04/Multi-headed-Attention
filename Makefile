...
# EXEC MPICXX INCLUDES MPI_CCFLAGS NVCC ALL_CCFLAGS GENCODE_FLAGS MPI_LDFLAGS LIBRARIES
# See the preivous homework Makefile for setting these variables
...
$(WORK_DIR)/load_model.o: $(SRC_DIR)/load_model.cu
	@mkdir -p $(WORK_DIR)
	$(NVCC) $(INCLUDES) -Iinclude -Isrc $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<
...
$(WORK_DIR)/load_tokens_cuda.o: $(SRC_DIR)/load_tokens.cpp
	@mkdir -p $(WORK_DIR)
	$(NVCC) $(INCLUDES) -Iinclude -Isrc $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

clean:
	rm -f $(SERIAL_TARGET) $(TOKENS_BIN) $(CUDA_TARGET) $(CUDA_CU_OBJS) $(CUDA_C_OBJS) $(MPI_TARGET) $(MPI_CUDA_TARGET) $(MPI_CUDA_CU_OBJS) $(MPI_CUDA_C_OBJS)
