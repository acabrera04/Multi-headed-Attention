...
// y = (x - mean(x)) / sqrt(var(x) + epsilon) * gamma + beta (where x is the
// input, gamma is the weight, beta is the bias)
typedef struct {
  float *weight;
  float *bias;
} LayerNorm;
...
// a transformer block stores layer norm, linear operations, and positional
// embeddings
...
// a model stores the token embeddings, positional embeddings, transformer
// blocks, and final layer norm
/*
  1. token embeddings
